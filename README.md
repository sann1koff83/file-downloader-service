...
ELMA365 отправляет HTTP POST-запрос с параметрами файла, который нужно скачать и сохранить на локальном сервере. Сервис принимает запрос, скачивает файл по указанной ссылке и сохраняет его в переданную папку.

## Возможности

- принимает HTTP API-запрос на скачивание файла;
- скачивает файл по `http` или `https`;
- создаёт целевую папку, если она отсутствует;
...
- `targetFolder` — путь к папке, куда нужно сохранить файл;
...
3. создаёт папку назначения, если её нет;
4. скачивает файл во временный `.part` файл;
5. после успешного скачивания переименовывает временный файл в итоговый;
6. возвращает JSON-ответ.
